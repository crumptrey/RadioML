...
y = zeros([2, windowLength, numFrames], class(x));
...
while startIdx + windowLength < numSamples
    ...
    realPart = real(xWindowed);
    imagPart = imag(xWindowed);

    % Normalize each part to have unit energy
    framePower = mean(abs(xWindowed).^2);
    realPart = realPart / sqrt(framePower);
    imagPart = imagPart / sqrt(framePower);

    % Save real and imaginary parts in the array
    y(1, :, frameCnt) = realPart;
    y(2, :, frameCnt) = imagPart;
    frameCnt = frameCnt + 1;
