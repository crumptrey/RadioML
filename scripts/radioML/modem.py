...
class Transmitter(object):
    ...
    After being constructed, the user can call modulate either with their own datasets
    (shown as "User Input" above) or they can pass in a set number of symbols in order
    to have "Random" datasets generated.  Either way, the output is always IQ at baseband.
    ...
    def modulate(self, bits: List[int] = None, n_symbols: int = 10000) -> torch.Tensor:
        """Modulate a provided list of bits (1s and 0s) or random datasets of a set length.

        If you wish to provide your own datasets, which is useful for later calculating the
        ...
        If instead you wish to simply get some quick datasets examples and don't care about
        ...
        Returns:
            torch.Tensor: Modulated datasets at baseband (1x1x2xn_symbols)
        """
        if bits is None and n_symbols <= 0:
            raise ValueError(
                "Must either provide datasets or a positive number of symbols to be faked "
        ...
        if bits is not None:
            ...
            for i, bit in enumerate(bits):
                ...
                if bit == 0:
                    ...
                    raise ValueError(
                        "Value of the passed in datasets at loc {} is not binary (e.g. "
...
class Receiver(object):
    ...
    After being constructed, the user can call demodulate either with their own IQ datasets
    (shown as "Baseband IQ Input" above) and this datasets is assumed to be formatted as
